...
nclass = max(data.y); % THIS MIGHT BE UNROBUST if some class number is not used!
...
for class1 = 1:nclass-1,
  for class2 = class1+1:nclass,
	...
	% CLASS SIZE OPTIMIZATION BY PAULI
	% Selecting only the amount of smaller group in classification (at
	% least 30 (80) though if possible, and max 500)
	
	data_inx1 = find(data.y==class1);
	data_inx2 = find(data.y==class2);
	
	d1=length(data_inx1);
	d2=length(data_inx2);
	
	datapoints1=min(min(max(min(d1,d2),5000),d1),5000); %put 500/500 here
	datapoints2=min(min(max(min(d1,d2),5000),d2),5000);
	
	dd1=randperm(d1);
	dd2=randperm(d2);
	
	indices1=data_inx1(dd1(1:datapoints1));
	indices2=data_inx2(dd2(1:datapoints2));
	data_inx=[indices1; indices2];
